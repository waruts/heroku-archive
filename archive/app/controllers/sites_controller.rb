class SitesController < ApplicationController  
  ...
  def download
    ...
    if different_domain
      if recursive
        if not system("wget", "-Q20m", "--timeout=3", "-pkH", "-r", "--level=#{recursive}", "-nd", "-P", "#{files_path}", *url_array)
          FileUtils.remove_dir base_path, true
          return render :action => :new  
        end
      else
        if not system("wget", "-Q20m", "--timeout=3", "-pkH", "-nd", "-P", "#{files_path}", *url_array)
          FileUtils.remove_dir base_path, true
          return render :action => :new
        end
      end
      ...
      if recursive
        if not system("wget", "-Q20m", "--timeout=3", "-pk", "-r", "--level=#{recursive}", "-nd", "-P", "#{files_path}", *url_array)
          FileUtils.remove_dir base_path, true
          return render :action => :new
        end
      else
        if not system("wget", "-Q20m", "--timeout=3", "-pk", "-nd", "-P", "#{files_path}", *url_array)
          FileUtils.remove_dir base_path, true
          return render :action => :new
        end
      end
